...
namespace zoneout {
    ...
    class Workspace {
      private:
        ...
        inline static UUID load_root_zone_id(const std::filesystem::path &workspace_json_path) {
            ...
            return parse_uuid_value(root_zone_elem->value);
        }
      ...
      public:
        ...
        inline void save(const std::filesystem::path &directory) const {
            ...
            write_uuid_json(out, "root_zone_id", root_zone_.id(), false);
            out << "}";
        ...
        inline static Workspace load(const std::filesystem::path &directory) {
            ...
            if (std::filesystem::exists(workspace_manifest) && std::filesystem::exists(zones_dir)) {
                const auto root_zone_id = load_root_zone_id(workspace_manifest);
                auto root_zone = load_zone_recursive(zones_dir, root_zone_id);
                auto graph = load_graph(graph_file);
                return Workspace(std::move(root_zone), std::move(graph));
            }
